'use client';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select';
import { Slider } from '@/components/ui/slider';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { TabsContent } from '@/components/ui/tabs';
import { useAppStore } from '@/stores/app-store';
import { useSelectedToken } from '@/hooks/queries/use-selected-token';
import { useSellTokens } from '@/hooks/mutations/use-buy-selected-token';
import Authenticated from '@/components/common/authenticated';
import RequestOTP from '@/components/login/request-otp';
import { useDialogStore } from '@/stores/dialog-store';
import { toast } from 'sonner';
import { Skeleton } from '@/components/ui/skeleton';
import { z } from 'zod';
import { zodResolver } from '@hookform/resolvers/zod';
import { useForm } from 'react-hook-form';
import { Form, FormControl, FormField, FormItem, FormMessage } from '@/components/ui/form';
import { useTokenBalance } from '@/hooks/queries/use-token-balance';
import { useWalletClient } from 'wagmi';
import { debounce } from 'lodash';
import { ethers } from 'ethers';
import { getQuote } from '@/utils/chain-client/txs/create_trade';
import { formatUnits } from 'ethers/lib/utils';

const PRESET_PERCENTAGES = [0, 25, 50, 75, 100] as const;
const MAX_DECIMALS = 2;

interface FormValues {
  amount: string;
  percentage: number;
}

const sellFormSchema = z.object({
  amount: z
    .string()
    .refine(val => !isNaN(Number(val)), { message: 'Invalid number' })
    .refine(val => Number(val) > 0, { message: 'Amount must be greater than 0' }),
  percentage: z.number().min(0).max(100),
});

export const SellTab = () => {
  const { open } = useDialogStore();
  const { setLoginEmail } = useAppStore();
  const { data: selectedToken } = useSelectedToken();
  const {
    data: selectedTokenBalance,
    fullBalance,
    isLoading: isBalanceLoading,
  } = useTokenBalance(selectedToken?.TokenA.Address || '', selectedToken?.TokenA.Decimals || 0);
  const sellTokensMutation = useSellTokens();
  const { data: wallet } = useWalletClient();
  const numericBalance = useMemo(() => Number(selectedTokenBalance || 0), [selectedTokenBalance]);
  const debouncedGetEstimatedUSDXRef = useRef<ReturnType<typeof debounce> | null>(null);
  const [estimatedUSDX, setEstimatedUSDX] = useState<number>(0);

  const form = useForm<FormValues>({
    resolver: zodResolver(sellFormSchema),
    defaultValues: {
      amount: '',
      percentage: 25,
    },
  });

  const { watch, setValue } = form;
  const amount = watch('amount');
  const percentage = watch('percentage');

  useEffect(() => {
    // Initialize the debounced function
    debouncedGetEstimatedUSDXRef.current = debounce(
      async (inputTokenAddress: string, outputTokenAddress: string, inputAmount: number) => {
        try {
          const quote = await getQuote({
            wallet: wallet as any,
            assetIn: inputTokenAddress,
            assetOut: outputTokenAddress,
            amount: inputAmount,
          });
          const estimatedValue =
            !isNaN(inputAmount) && inputAmount > 0 ? ethers.utils.formatUnits(quote, 18) : 0;
          setEstimatedUSDX(Number(estimatedValue));
        } catch (error) {
          console.error('Error getting estimated USDX:', error);
        }
      },
      500
    );

    // Cleanup function
    return () => {
      if (debouncedGetEstimatedUSDXRef.current) {
        debouncedGetEstimatedUSDXRef.current.cancel();
      }
    };
  }, [
    selectedToken?.TokenB.Decimals,
    selectedToken?.TokenA.Address,
    selectedToken?.TokenB.Address,
    wallet,
  ]);

  // Format a number to have at most MAX_DECIMALS decimal places
  const formatNumber = (num: number): string => {
    return num.toFixed(MAX_DECIMALS).replace(/\.?0+$/, '');
  };

  // Handle amount input changes
  const handleAmountChange = useCallback(
    (value: string) => {
      // Allow empty input
      if (!value) {
        setValue('amount', '');
        setValue('percentage', 0);
        return;
      }

      // Clean and format the input value
      const cleanValue = value.replace(/[^0-9.]/g, '');
      const parts = cleanValue.split('.');
      const formattedValue =
        parts[0] + (parts.length > 1 ? '.' + parts[1].slice(0, MAX_DECIMALS) : '');

      let numValue = Number(formattedValue);
      if (isNaN(numValue)) return;

      const fullBalanceNumber = Number(formatUnits(fullBalance ?? 0, 18));
      if (fullBalanceNumber && numValue > fullBalanceNumber) {
        numValue = fullBalanceNumber;
      }

      // Update form values
      setValue('amount', numValue.toString());
      setValue('percentage', Math.min(100, (numValue / numericBalance) * 100));

      if (
        selectedToken?.TokenA.Address &&
        selectedToken?.TokenB.Address &&
        debouncedGetEstimatedUSDXRef.current
      ) {
        debouncedGetEstimatedUSDXRef.current(
          selectedToken.TokenA.Address,
          selectedToken.TokenB.Address,
          numValue
        );
      }
    },
    [
      selectedToken?.TokenA.Address,
      selectedToken?.TokenB.Address,
      debouncedGetEstimatedUSDXRef,
      numericBalance,
      setValue,
      fullBalance,
    ]
  );

  // Update amount when percentage changes
  useEffect(() => {
    const newAmount = numericBalance * (percentage / 100);
    // setValue('amount', formatNumber(newAmount));
    handleAmountChange(formatNumber(newAmount));
  }, [percentage, numericBalance, handleAmountChange]);

  const isInsufficientBalance = useMemo(() => {
    if (!amount) return false;
    const numAmount = Number(amount);
    return !isNaN(numAmount) && numAmount > numericBalance;
  }, [amount, numericBalance]);

  const isValidAmount = useMemo(() => {
    if (!amount) return false;
    const numAmount = Number(amount);
    return !isNaN(numAmount) && numAmount > 0;
  }, [amount]);

  const onSubmit = (values: FormValues) => {
    if (isInsufficientBalance) return toast.error('Insufficient balance');
    if (!isValidAmount) return toast.error('Amount must be greater than 0');

    const numAmount = Number(values.amount);
    sellTokensMutation.mutate({
      wallet: wallet as any,
      assetIn: selectedToken?.TokenA.Address || '',
      assetInDecimals: selectedToken?.TokenA.Decimals || 0,
      assetOut: selectedToken?.TokenB.Address || '',
      amount: formatNumber(numAmount),
    });
  };

  return (
    <TabsContent value="sell">
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className="flex flex-col gap-8 pt-8 text-muted-foreground"
        >
          <div className="flex flex-col gap-2">
            <p className="text-sm">Token</p>
            <Select>
              <SelectTrigger className="w-full">
                <SelectValue placeholder={selectedToken?.TokenA.Name} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={selectedToken?.TokenA.Name || ''}>
                  {selectedToken?.TokenA.Name}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <p className="text-sm">Amount</p>
              <p className="text-sm text-muted-foreground">
                Balance: {formatNumber(numericBalance)} {selectedToken?.TokenA.Name}
              </p>
            </div>
            <FormField
              control={form.control}
              name="amount"
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
                      placeholder="0.00"
                      type="text"
                      inputMode="decimal"
                      value={field.value}
                      onChange={e => handleAmountChange(e.target.value)}
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
          </div>

          <div className="flex flex-col gap-2 pt-4">
            <FormField
              control={form.control}
              name="percentage"
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Slider
                      value={[field.value]}
                      onValueChange={([value]) => setValue('percentage', value)}
                      max={100}
                      step={1}
                    />
                  </FormControl>
                </FormItem>
              )}
            />
            <div className="flex justify-between">
              {PRESET_PERCENTAGES.map(percent => (
                <div
                  key={percent}
                  onClick={() => setValue('percentage', percent)}
                  className="cursor-pointer hover:text-primary"
                >
                  {percent === 100 ? 'Max' : `${percent}%`}
                </div>
              ))}
            </div>
          </div>

          <Authenticated
            customUI={
              <Button
                type="button"
                variant="secondary"
                onClick={() => {
                  open({
                    title: 'Login to your ODX account',
                    component: <RequestOTP />,
                    size: 'md',
                  });
                  setLoginEmail('');
                }}
              >
                Login or Signup
              </Button>
            }
          >
            <Button
              type="submit"
              disabled={sellTokensMutation.isPending || !isValidAmount || isInsufficientBalance}
              isLoading={sellTokensMutation.isPending}
              variant={isInsufficientBalance ? 'destructive' : 'default'}
            >
              {isInsufficientBalance
                ? 'Insufficient Balance'
                : isValidAmount
                  ? 'Sell'
                  : 'Enter Amount'}
            </Button>
          </Authenticated>

          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <p>{selectedToken?.TokenA.Name} Balance</p>
              <div>
                {isBalanceLoading ? (
                  <Skeleton className="h-4 w-20" />
                ) : (
                  formatNumber(numericBalance)
                )}
              </div>
            </div>
            <div className="flex items-center justify-between">
              <p>{selectedToken?.TokenB.Name} (estimated)</p>
              <p>{formatNumber(estimatedUSDX)}</p>
            </div>
            <div className="flex items-center justify-between">
              <p>Order Value</p>
              <p>
                {amount
                  ? `${formatNumber(Number(amount))} ${selectedToken?.TokenA.Name}`
                  : `0.00 ${selectedToken?.TokenA.Name}`}
              </p>
            </div>
          </div>
        </form>
      </Form>
    </TabsContent>
  );
};
